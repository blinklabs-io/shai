...
func (a *LendingOracleAPI) RegisterHandlers(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/v1/lending/markets", a.HandleListMarkets)
	mux.HandleFunc("GET /api/v1/lending/markets/{marketId}", a.HandleGetMarket)
	mux.HandleFunc("GET /api/v1/lending/loans", a.HandleListLoans)
	mux.HandleFunc("GET /api/v1/lending/loans/{loanId}", a.HandleGetLoan)
	mux.HandleFunc("GET /api/v1/lending/rates", a.HandleListRates)
	mux.HandleFunc("GET /api/v1/lending/utilization", a.HandleListUtilization)
	mux.HandleFunc("GET /api/v1/lending/overdue", a.HandleListOverdueLoans)
	mux.HandleFunc("GET /ws/lending", a.HandleLendingStream)
}
		...
		for {
			...
			case <-ticker.C:
				if err := conn.writePing(); err != nil {
					logger.Debug(
						"failed to ping lending WebSocket client",
						"error", err,
						"remote", raw.RemoteAddr(),
					)
					conn.close()
