...
const (
	...
	LendingStateTypeOracle                            // Price oracle feed
)
...
type LendingState struct {
	...
	// Oracle State (for protocol oracle feed UTxOs)
	OraclePrice       uint64    `json:"oraclePrice,omitempty"`
	OracleDenominator uint64    `json:"oracleDenominator,omitempty"`
	OraclePriceValue  float64   `json:"oraclePriceValue,omitempty"`
...
func NewLendingUpdate(state *LendingState) *LendingUpdate {
	return &LendingUpdate{
		...
		OraclePrice:     state.OraclePriceValue,
...
func (o *LendingOracle) Start() error {
	...
	if o.storage == nil {
		return fmt.Errorf("lending storage is required")
	}
...
func (o *LendingOracle) handleRollback(evt event.RollbackEvent) error {
	...
	for _, state := range toDelete {
		...
		if err := o.storage.DeleteLendingState(
			state.Network,
			state.Protocol,
			state.StateId,
		); err != nil {
			logger.Error(
				"failed to delete rolled-back lending state",
				"error", err,
				"stateId", state.StateId,
				"slot", state.Slot,
			)
		...
		logger.Info(
			"invalidated lending state due to rollback",
			"stateId", state.StateId,
			"slot", state.Slot,
	...
	if len(errs) > 0 {
		return fmt.Errorf(
			"failed to delete %d lending states during rollback",
			len(errs),
		)
	}
	return nil
...
func (o *LendingOracle) loadPersistedStates() error {
	...
	states, err := o.storage.LoadLendingStatesByProtocol(
