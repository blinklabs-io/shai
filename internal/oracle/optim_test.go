...
func TestOptimRational(t *testing.T) {
	...
	rationalConstr := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
...
func TestOptimBondDatumFull(t *testing.T) {
	...
	paymentCred := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
		pubKeyHash,
	})

	// Stake credential: None
	stakeCred := cbor.NewConstructorEncoder(1, cbor.IndefLengthList{})

	// Address
	address := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
		paymentCred,
		stakeCred,
	})
	...
	bondDatum := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
		bondNFT,
		address,
		borrowerNFT,
...
func TestOptimOADADatumFull(t *testing.T) {
	...
	exchangeRate := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
		uint64(105), // 1.05 exchange rate
		uint64(100),
	})

	// OADADatum
	oadaDatum := cbor.NewConstructorEncoder(0, cbor.IndefLengthList{
