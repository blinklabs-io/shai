...
func TestOrderTypeAddLiquidity(t *testing.T) {
	...
	if details.Type != OrderTypeAddLiquidity {
...
func TestOrderTypeTradeAToB(t *testing.T) {
	...
	if details.Type != OrderTypeTradeAToB {
...
func TestOrderTypeTradeBToA(t *testing.T) {
	...
	if details.Type != OrderTypeTradeBToA {
...
func TestOrderTypeServe(t *testing.T) {
	...
	if details.Type != OrderTypeServe {
...
func TestParserParseAssetId(t *testing.T) {
	parser := NewParser()

	// Test concatenated hex format (current format from UTXO)
	// Policy ID: 28 bytes (56 hex chars), Name: 16 bytes (32 hex chars)
	assetId := "1234567890abcdef1234567890abcdef1234567890abcdef12345678" + // 56 chars policy ID
		"fedcba0987654321fedcba0987654321" // 32 chars name
	expectedPolicyId := []byte{
		0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef,
		0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef,
		0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef,
		0x12, 0x34, 0x56, 0x78, // 28 bytes total
	}
	expectedName := []byte{
		0xfe, 0xdc, 0xba, 0x09, 0x87, 0x65, 0x43, 0x21,
		0xfe, 0xdc, 0xba, 0x09, 0x87, 0x65, 0x43, 0x21, // 16 bytes
	}

	asset, err := parser.parseAssetId(assetId)
	if err != nil {
		t.Fatalf("failed to parse concatenated asset ID: %v", err)
	}
	if string(asset.PolicyId) != string(expectedPolicyId) {
		t.Errorf("expected policy ID %x, got %x", expectedPolicyId, asset.PolicyId)
	}
	if string(asset.Name) != string(expectedName) {
		t.Errorf("expected name %x, got %x", expectedName, asset.Name)
	}

	// Test dot-delimited format
	dotAssetId := "1234567890abcdef1234567890abcdef1234567890abcdef12345678.fedcba0987654321fedcba0987654321"
	dotAsset, err := parser.parseAssetId(dotAssetId)
	if err != nil {
		t.Fatalf("failed to parse dot-delimited asset ID: %v", err)
	}
	if string(dotAsset.PolicyId) != string(expectedPolicyId) {
		t.Errorf("expected policy ID %x, got %x", expectedPolicyId, dotAsset.PolicyId)
	}
	if string(dotAsset.Name) != string(expectedName) {
		t.Errorf("expected name %x, got %x", expectedName, dotAsset.Name)
	}

	// Test NFT format (policy ID only, concatenated)
	nftAssetId := "abcdef1234567890abcdef1234567890abcdef1234567890abcdef12" // 56 chars = 28 bytes
	nftAsset, err := parser.parseAssetId(nftAssetId)
	if err != nil {
		t.Fatalf("failed to parse NFT asset ID: %v", err)
	}
	expectedNftPolicyId := []byte{
		0xab, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x90,
		0xab, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x90,
		0xab, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x90,
		0xab, 0xcd, 0xef, 0x12, // 28 bytes
	}
	if string(nftAsset.PolicyId) != string(expectedNftPolicyId) {
		t.Errorf("expected NFT policy ID %x, got %x", expectedNftPolicyId, nftAsset.PolicyId)
	}
	if len(nftAsset.Name) != 0 {
		t.Errorf("expected empty name for NFT, got %x", nftAsset.Name)
	}
}
