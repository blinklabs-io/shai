...
func TestBuildRedeemerPlutusData_CompleteFill(t *testing.T) {
	...
	constr, ok := redeemerData.Value.(cbor.ConstructorEncoder)
	if !ok {
...
func TestBuildOrderRedeemer_PartialFill(t *testing.T) {
	...
	if constr.Tag() != 0 {
		t.Errorf("expected constructor 0, got %d", constr.Tag())
	}
}
...
func TestBuildUpdatedOrderDatum(t *testing.T) {
	...
	constr, ok := datum.Value.(cbor.ConstructorEncoder)
	if !ok {
