...
// Identifier returns a stable Cardano credential identifier.
func (c *Credential) Identifier() string {
	switch c.Type {
	case 0:
		return fmt.Sprintf("pubkey:%x", c.Hash)
	case 1:
		return fmt.Sprintf("script:%x", c.Hash)
	default:
		return fmt.Sprintf("credential%d:%x", c.Type, c.Hash)
	}
}
...
func availableLiquidity(totalSupply, totalBorrows, reserveAmount uint64) uint64 {
	if totalSupply <= totalBorrows {
		return 0
	}
	liquid := totalSupply - totalBorrows
	if liquid <= reserveAmount {
		return 0
	}
	return liquid - reserveAmount
}

func basisPointsFloat(value uint64) float64 {
	return float64(value) / float64(BasisPointsDenom)
}
